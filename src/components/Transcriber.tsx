...
export default function Transcriber() {
  const {
    transcripts, listening, interim, language, setLanguage,
    lyttAvailable, isProcessing, error, browserSupported,
    toggle, deleteTranscript, clearAll,
  } = useTranscriber()

  const canUse = lyttAvailable || browserSupported
  const engineLabel = lyttAvailable ? 'lytt · Whisper' : 'Browser Speech API'
  const engineStatus = lyttAvailable === null ? 'checking…' : lyttAvailable ? 'connected' : 'fallback'
      ...
      <div className={styles.inner}>
        ...
        {!canUse && (
          <p className={styles.unsupported}>
            No transcription engine available. Run <code>lytt serve</code> locally, or use Chrome/Edge for browser fallback.
          </p>
        )}
          ...
          <>
            ...
            {interim && (
              <p className={styles.interim}>
                {interim}<span className={styles.cursor}>|</span>
              </p>
            )}
            ...
            {transcripts.length > 0 && (
              <div className={styles.list}>
                <div className={styles.listHeader}>
                  <span>{transcripts.length} {transcripts.length === 1 ? 'entry' : 'entries'}</span>
                ...
                {transcripts.map((t) => (
                  <div key={t.id} className={styles.entry}>
                    ...
                    <div className={styles.entryMeta}>
                      <span className={styles.entrySource} data-source={t.source}>
            ...
            {!lyttAvailable && (
              <p className={styles.lyttHint}>
                ...
                <a href="https://github.com/Smebbs/lytt" target="_blank" rel="noopener noreferrer">lytt</a>
